...
void loop(){
    ...
    if (loopCounter == 8){
        ...
        display.setTextSize(1);
        display.println(F("MAGS Pitch  Roll Temp"));
        display.println(displayStr);
        display.display();
        if(loops == 2){
          loops = 0;

          checkAS5600Setup();                  // check the magnet (blocks until magnet is found)
...
void connectWiFi(){
    if (WiFi.waitForConnectResult() != WL_CONNECTED) {  
        if (try1stPassword) {
          Serial.println("WiFi Failed - password 1");   
          WiFi.begin(ssid, password);
        } else {
          Serial.println("WiFi Failed - password 2");   
...
void update(){
    ...
    helm_string += buff;
    dtostrf(Heading, 5, 1, buff);
    headingStr = buff; 

    dtostrf(desiredHeading, 1, 1, buff);
    helm_string += ",D:"+String(buff);
    dtostrf(desiredHeading, 5, 1, buff);
    desiredHeadingStr = buff; 

    dtostrf(abs(courseError), 3, 0, buff);
...
float ret = -1;
    ...
    if (AS5600Setup && desiredHeading >= 0){
      ...
      ret = (rawAngle + absRot * 4096) * 0.0219726;
    }
#endif
...
#ifdef AS5600
// ----------------------
// AS5600 Rotation sensor  Code - could be used 
// ----------------------
...
void checkMagnetPresence(){
  ...
  if ((magnetStatus & B00100000) != 32) {                   // locks MCU until magnet correctly positioned
